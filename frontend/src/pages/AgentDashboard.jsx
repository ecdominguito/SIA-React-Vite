...
export default function AgentDashboard() {
  ...
  const notifyCustomerForAppointment = (appointment, status, context = {}) => {
    ...
    let message = "";
    if (status === "approved") {
      message = `Agent @${user?.username} confirmed ${propertyLabel} on ${nextDate} at ${nextTime}.`;
    } else if (status === "rescheduled") {
      const previousSlot = previousDate && previousTime ? ` from ${previousDate} ${previousTime}` : "";
      message = `Agent @${user?.username} rescheduled ${propertyLabel}${previousSlot} to ${nextDate} ${nextTime}.`;
    } else if (status === "cancelled") {
      message = `Agent @${user?.username} cancelled ${propertyLabel} scheduled on ${nextDate} at ${nextTime}.`;
    } else {
      return;
    }
  ...
  const notifyCustomerForMeet = (meet, status) => {
    ...
    pushNotification({
      ...
      message: `Agent @${user?.username} marked your ${modeLabel} office meet as ${statusLabel} (${date} ${time}).`,
      meta: {
        meetId: meet?.id || "",
        status,
        date,
  ...
  const mineApps = useMemo(() => apps.filter((a) => a.agent === user?.username), [apps, user]);
                ...
                {mineApps.slice(0, 4).map((a) => (
                  <div className="agent-mini-row" key={a.id}>
                    <div>
                      ...
                      <div className="small muted">{a.date} at {a.time}</div>
                    </div>
                  ...
                  >
                    <div className="row g-2">
                      ...
                      <div className="col-md-6">
                        ...
                        <input className="form-control" type="date" value={tForm.date} onChange={(e) => setTForm((s) => ({ ...s, date: e.target.value }))} />
                      </div>
                      <div className="col-md-6">
                        <label className="form-label">Time *</label>
                        <input className="form-control" type="time" value={tForm.time} onChange={(e) => setTForm((s) => ({ ...s, time: e.target.value }))} />
                      </div>
                      <div className="col-12">
